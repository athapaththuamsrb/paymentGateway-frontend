...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.timeStamp = year + "" + monthSt + "" + daySt + "" + hourSt + "" + minutesSt + "" + secoundSt
    const merchantId = "dev207328675986663817";
    const sharedSecret = "pE2K6ONAy2";
    const hashSquance1 = `${this.timeStamp}.${merchantId}.${"N6qsk4kYRZihmPrTXWYS6g"}.${"1999"}.${"EUR"}`;
    const hash1 = sha1(hashSquance1);
    const hashSquance2 = `${hash1}.${sharedSecret}`;
    this.sha1hash = sha1(hashSquance2);
  }
